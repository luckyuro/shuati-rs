...
impl Solution {
    pub fn three_sum_multi(a: Vec<i32>, target: i32) -> i32 {
        ...
        for p_anchor in 0..num_once.len() {
            ...
            let mut p_start: usize = if num_times[&num_once[p_anchor]] > 1 {
                p_anchor
            } else {
                p_anchor + 1
            };
                ...
                match (num_once[p_start] + num_once[p_end]).cmp(&target) {
                    ...
                    Ordering::Equal => {
                        if p_start == p_end && p_end == p_anchor {
                            ...
                            answer += combination(num_times[&num_once[p_start]], 2).unwrap()
                                * num_times[&num_once[p_end]];
                            ...
                            answer += num_times[&num_once[p_anchor]]
                                * num_times[&num_once[p_start]]
                                * num_times[&num_once[p_end]];
                        };
                        answer %= limit;
                        p_start += 1;
